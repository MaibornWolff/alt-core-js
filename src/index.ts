...
export const runScenario = (scenarioPath: string, actionDir: string, envConfigFile: string) => {
    try {
        ...
        const scenarios: Scenario[] = scenarioPath.endsWith('yaml') ? loadScenariosById(scenarioPath, actions) : loadAllScenarios(scenarioPath, actions);
...
async function processScenarios(scenarios: Scenario[]) {
    ...
    while (scenarios.length > 0) {
        await Promise.all(scenarios.splice(0, NUM_PARALLEL_RUNS).map(invokeActionsSynchronously));
    }
    printResults();
...
async function invokeActionsSynchronously(scenario: Scenario) {
    ...
    getLogger(scenarioName).debug(pad(`#### (S): ${scenarioName}: ${scenario.description} `, MSG_WIDTH, '#'), ctx);
    getLogger(scenarioName).debug(pad(MSG_WIDTH, '#', '#'), ctx);
    ...
    for (let action of scenario.actions) {
        ...
        getLogger(scenarioName).info(pad(`#### (A): ${action.name} `, MSG_WIDTH, '#'), ctx);
        ...
        .then((result) => {
            ...
            if (scenarioResults) scenarioResults.push(new TestResult(action.name, duration, true));
            
            if (result) getLogger(scenario.name).debug(JSON.stringify(result), ctx);
            getLogger(scenario.name).info(pad(MSG_WIDTH, ` Time: ${duration} ms ###########`, '#'), ctx);
        })
        .catch((reason) => {
            let duration = timeDiffInMs(process.hrtime(start)).toFixed(2);
            
            let scenarioResults = RESULTS.get(scenarioName);
            if (scenarioResults) scenarioResults.push(new TestResult(action.name, duration, false));
...
function printResults(): any {
    ...
    RESULTS.forEach((result, scenario) => {
        ...
        getLogger(scenario).info(pad(`#### SUMMARY: ${scenario} `, MSG_WIDTH, '#'), ctx);
        
        result.forEach((res: TestResult) => {
            if (res.successful)
            getLogger(scenario).info(` OK: ${pad(res.action, 30)} ${res.duration} ms`, ctx);
            else
            getLogger(scenario).info(`NOK: ${pad(res.action, 30)} ${res.duration} ms`, ctx);
        });
