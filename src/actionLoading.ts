...
export const loadAllActions = (actionDir: string, envConfig: any): Action[] => {
    ...
    readdirSync(actionDir).forEach((file: any) => {
        ...
        if (isRestAction(actionDef)) {
            ...
            loadedActions.push(new RestAction(nameFromYamlConfig(file), actionDef, host + actionDef.endpoint, actionDef.service));

        } else if (isTimerAction(actionDef)) {
            loadedActions.push(new TimerAction(nameFromYamlConfig(file), actionDef));

        } else if (isWebsocketAction(actionDef)) {
            loadedActions.push(new WebSocketAction(nameFromYamlConfig(file), actionDef, actionDef.service, 'wss://' + envConfig[actionDef.service] + actionDef.endpoint));

        } else if (isMqttAction(actionDef)) {
            loadedActions.push(new MqttAction(nameFromYamlConfig(file), actionDef));
