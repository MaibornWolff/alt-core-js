...
export const loadScenariosById = (path: string, actionCatalog: Action[]): Scenario[] => {
    let resultList: Scenario[] = [];
    let scenarioName = path.split('/').pop().replace('.yaml', '');
    ...
    if (resultList.length > 0) {
        ...
        getLogger("unknown").error(`Scenario '${scenarioName}' not found in the directory!`);
        process.exit(1);
    }
};

export const loadAllScenarios = (path: string, actionCatalog: Action[]): Scenario[] => {
        ...
        if (scenarioDef) {
            ...
            if (scenarioDef.loadFactor) {
                for (let _i = 0; _i < scenarioDef.loadFactor; _i++) {
                    let scenarioNameWithIdx = nameFromYamlConfig(file) + "-" + _i;
                    ...
                    if (scenarioDef.loadVariables) {
                        let currentLoadVariables = getLoadVariableTreeForLoadIdx(scenarioDef.loadVariables, _i);
                        for (let _current of Object.entries(currentLoadVariables)) {
                            let currentLoad = _current[1];

                            let actionToBeReplaced: any = actionCatalogWithReplacedLoadVariables.find(a => a.name === (currentLoad as any).name);
                                ...
                                for (let key of Object.keys(currentLoad)) {
                                    if (key !== 'name') {
                                        if (actionToBeReplaced[key] && typeof actionToBeReplaced[key] === 'object') {
                                            getLogger(scenarioNameWithIdx).debug(`Replacing "${stringify(actionToBeReplaced[key])}" with "${stringify(currentLoad[key])}" for key "${key}"`, Object.assign(ctx, {action: actionToBeReplaced.name}));
                                            Object.assign(actionToBeReplaced[key], currentLoad[key]);
                                        } else if (actionToBeReplaced[key]) {
                                            getLogger(scenarioNameWithIdx).debug(`Replacing "${actionToBeReplaced[key]}" with "${currentLoad[key]}" for key "${key}"`, Object.assign(ctx, {action: actionToBeReplaced.name}));
                    ...
                    loadedScenarios.push(
                        new Scenario(scenarioNameWithIdx, scenarioDef, actionCatalogWithReplacedLoadVariables)
                    )
                }
            } else {
                // just one instance of the scenario
                loadedScenarios.push(
                    new Scenario(nameFromYamlConfig(file), scenarioDef, actionCatalog)
                );
            }
    ...
    for (let propEntry of Object.entries(rootObject)) {
        ...
        if (Array.isArray(propEntry[1])) {
            ...
            if (loadVars.length == 1 && loadVars[0].indexOf("...") >= 0) {
                let stringPrefix = loadVars[0].substr(0, loadVars[0].indexOf("$$"));
                let numberRange = loadVars[0].replace("$$", "").substring(stringPrefix.length);
                let limits = numberRange.split("...");
                let minValue = Number.parseInt(limits[0]);
                let maxValue = Number.parseInt(limits[1]);
                let offset = (minValue + idx) <= maxValue ? idx : (minValue + idx - maxValue);
