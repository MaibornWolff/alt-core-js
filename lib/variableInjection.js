...
function injectEvalAndVarsToMap(keyValueMap, scenarioVariables, loggingCtx) {
    var copy = keyValueMap instanceof Array ? [] : {};
...
function buildExpHelpers(vars) {
    return {
        ...
        getAndIncBy: function (name, howMuch) {
            var currentValue = vars.get(name);
        ...
        incByAndGet: function (name, howMuch) {
            var targetValue = vars.get(name) + howMuch;
            vars.set(name, targetValue);
            return targetValue;
        },
        ...
        timestampPlusMinutes: function (minutes) {
