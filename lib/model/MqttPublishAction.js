...
var MqttPublishAction = /** @class */ (function () {
    ...
    MqttPublishAction.prototype.encodeProtoPayload = function (scenarioVariables, ctx) {
        if (ctx === void 0) { ctx = {}; }
        var data = variableInjection_1.injectEvalAndVarsToMap(this.data, scenarioVariables, ctx);
        return [protoParsing_1.encodeProto(this.protoFile, data, this.protoClass), JSON.stringify(data)];
    };
    MqttPublishAction.prototype.invokeAsync = function (scenario) {
        ...
        client.on('connect', function () {
            ...
            var dataString; // a JSON-string injected with read data, used for logging
            var payload; // JSON-string or binary payload
            if (_this.protoFile) {
                // protobuf binary data
                _a = _this.encodeProtoPayload(scenario.cache, ctx), payload = _a[0], dataString = _a[1];
            }
            else {
                // normal JSON data
                payload = JSON.stringify(variableInjection_1.injectEvalAndVarsToMap(_this.data, scenario.cache, ctx));
                dataString = payload;
            }
            var topic = variableInjection_1.injectEvalAndVarsToString(_this.topic, scenario.cache, ctx).toString();
