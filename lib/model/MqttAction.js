...
var MqttAction = /** @class */ (function () {
    function MqttAction(name, mqttDefinition, url, username, password, topic, durationInSec, expectedNumberOfMessages, messageType, messageFilter) {
    ...
    MqttAction.prototype.invokeAsync = function (scenario) {
        ...
        var isMessageRelevant = function (message) {
        ...
        client.on('message', function (topic, message) {
            if (isMessageRelevant(message)) {
                ...
                logDebug("Relevant MQTT update received (" + numberOfRetrievedMessages + "/" + _this.expectedNumberOfMessages + "): " + message.toString());
                diagramDrawing_1.addMqttMessage(scenario.name, topic, message.toString());
            }
            else {
