...
describe('string injection', function () {
    ...
    it('should not convert a digit-only string to a number if it does not contain a numeric-expression', function () {
        var result = variableInjection_1.injectEvalAndVarsToString('99999999', new Map(), {});
        chai_1.expect(result).to.equal('99999999');
    });
    it('should be able to inject use helper methods inside expressions', function () {
        var variableMap = new Map();
        variableMap.set('v', 3);
        var resultSetGet = variableInjection_1.injectEvalAndVarsToString('<<< this.set("v", 4); this.get("v") >>>', variableMap, {});
        chai_1.expect(resultSetGet).to.equal(4);
        var resultGetAndInc = variableInjection_1.injectEvalAndVarsToString('<<< this.getAndInc("v") >>>', variableMap, {});
        chai_1.expect(resultGetAndInc).to.equal(4);
        var resultIncAndGet = variableInjection_1.injectEvalAndVarsToString('{{{ this.incAndGet("v") }}}', variableMap, {});
        chai_1.expect(resultIncAndGet).to.equal("6");
    });
